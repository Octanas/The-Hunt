...
public class PreyMovement extends TickerBehaviour {
	...
	protected void onTick() {
		...
		if (maze != null) {
			...
			// If there is a visible hunter, mark the movement in the direction it was seen to be avoided for 5 ticks
			...
			// Order the movements from best to worst, according to the number of ticks that it should be avoided
			//
			// Movements with the same number of ticks that they should be avoided are randomised in the order between them
			// A movement to go backwards will always be worse than another movement with the same number of ticks that it should be avoided
			for (int i = 0; i < possibleMovements.size() - 1; i++) {
				for (int j = 0; j < possibleMovements.size() - i - 1; j++) {
					if (movementsToAvoid.get(possibleMovements.get(j)) > movementsToAvoid.get(possibleMovements.get(j + 1))
							|| movementsToAvoid.get(possibleMovements.get(j)) == movementsToAvoid.get(possibleMovements.get(j + 1))
									&& (direction.equals(Maze.Direction.North) && possibleMovements.get(j).equals(Maze.Movement.Down)
											|| direction.equals(Maze.Direction.South) && possibleMovements.get(j).equals(Maze.Movement.Up)
											|| direction.equals(Maze.Direction.East) && possibleMovements.get(j).equals(Maze.Movement.Left)
											|| direction.equals(Maze.Direction.West) && possibleMovements.get(j).equals(Maze.Movement.Right)
											|| rand.nextBoolean()) 
										&& !(direction.equals(Maze.Direction.North) && possibleMovements.get(j + 1).equals(Maze.Movement.Down)
													|| direction.equals(Maze.Direction.South) && possibleMovements.get(j + 1).equals(Maze.Movement.Up)
													|| direction.equals(Maze.Direction.East) && possibleMovements.get(j + 1).equals(Maze.Movement.Left)
													|| direction.equals(Maze.Direction.West) && possibleMovements.get(j + 1).equals(Maze.Movement.Right))) {
			...
			// Follow the list until a movement is valid (the worst movement will always be the last one to be tried)
